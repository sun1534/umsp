...
import com.partsoft.utils.BitUtils;
import com.partsoft.utils.HexUtils;

public abstract class AbstractBuffer implements Buffer {

	protected final static String __IMMUTABLE = "IMMUTABLE", __READONLY = "READONLY", __READWRITE = "READWRITE",
			__VOLATILE = "VOLATILE";
	...
	public int getInt() {
		int result = BitUtils.getInt(array(), _get);
		_get += INT_SIZE;
		return result;
	}

	public long getLong() {
		long result = BitUtils.getLong(array(), _get);
		_get += LONG_SIZE;
		return result;
	}

	public double getDouble() {
		double result = BitUtils.getDouble(array(), _get);
		_get += DOUBLE_SIZE;
		return result;
	}

	public float getFloat() {
		float result = BitUtils.getFloat(array(), _get);
		_get += FLOAT_SIZE;
		return result;
	}

	public short getShort() {
		short result = BitUtils.getShort(array(), _get);
		_get += SHORT_SIZE;
		return result;
	}

	public char getChar() {
		char result = BitUtils.getChar(array(), _get);
		_get += CHAR_SIZE;
		return result;
	}

	public void putInt(int value) {
		put(BitUtils.toBytes(value));
	}
	...
	public String toAllDetailString() {
		...
		if (markIndex() >= 0) {
			for (int i = markIndex(); i < getIndex(); i++) {
				buf.append(HexUtils.toHex(peek(i)));
				buf.append(' ');
			}
			buf.append("}{");
		}
