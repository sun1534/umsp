...
import com.partsoft.umsp.io.RuntimeIOException;
...
public class PacketConnection implements Connection {
	...
	public String getProtocol() {
		if (_protocol == null) {
			_protocol = _connector != null ? _connector.getProtocol() : "UNKOWN";
		}
		return _protocol;
	}
		...
		try {
			...
			while (more_in_buffer) {
				try {
					...
					if (e.getCause() instanceof SocketTimeoutException && _parser.isIdle()) {
						handleTimeout();
					} else {
						_parser.reset(true);
						_endp.close();
